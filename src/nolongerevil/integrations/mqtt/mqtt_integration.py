...
class MqttIntegration(BaseIntegration):
    ...
    def __init__(
        ...
        self._minimal_discovery = self.get_config_value("minimalDiscovery", False)
    ...
    async def _publish_ha_state(
        ...
        # HVAC action
        action = derive_hvac_action(device_values, shared_values, structure_values)
        await client.publish(
        ...
        # Occupancy (aligned with eco/away presets)
        is_away = is_device_away(device_values, structure_values)
        await client.publish(
    ...
    async def _publish_discovery(self, client: aiomqtt.Client, serial: str) -> None:
        ...
        configs = get_all_discovery_configs(
            ...
            minimal_discovery=self._minimal_discovery,
        )
